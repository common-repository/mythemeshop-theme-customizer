...
/**
 * Class onepage_team_widget
 */
if ( !class_exists( 'onepage_team_widget' ) ) {

	class onepage_team_widget extends WP_Widget {

		/**
		 * onepage_team_widget constructor.
		 */
		public function __construct() {
			parent::__construct(
				'onepage_team-widget',
				__( 'onepage - Team member widget', 'mythemeshop-theme-customizer' ),
				array(
		...
		function widget_scripts( $hook ) {
			...
			wp_enqueue_media();
			wp_enqueue_script( 'onepage_widget_media_script', MTS_CUSTOMIZER_URL . 'assets/js/widget-media.js', false, '1.1', true );
		}
			...
			<div class="col-lg-3 col-sm-3 team-box">
				...
				<div class="team-member" tabindex="0">
					...
					<?php if ( ! empty( $instance['image_uri'] ) && ( $instance['image_uri'] != 'Upload Image' ) ) { ?>
						...
						$onepage_team_custom_media_id = wp_get_attachment_image_src( $instance["custom_media_id"] );
						$alt                        = get_post_meta( $instance['custom_media_id'], '_wp_attachment_image_alt', true );

						if ( ! empty( $onepage_team_custom_media_id ) && ! empty( $onepage_team_custom_media_id[0] ) ) {
							?>

							<figure class="profile-pic">

								<img src="<?php echo esc_url( $onepage_team_custom_media_id[0] ); ?>"
								     alt="<?php echo $alt; ?>"/>
					...
					<div class="social-icons">
						...
						<ul>
							<?php
							$onepage_team_target = '_self';
							if ( ! empty( $instance['open_new_window'] ) ):
								$onepage_team_target = '_blank';
							endif;
							?>
							...
							if ( ! empty( $instance['fb_link'] ) ): ?>
								<li>
									<a href="<?php echo apply_filters( 'widget_title', $instance['fb_link'] ); ?>"
									   target="<?php echo $onepage_team_target; ?>">
										<?php
										if ( ! empty( $instance['name'] ) ) { ?>
											<span class="sr-only">
				                            <?php _e( 'Facebook account of', 'mythemeshop-theme-customizer' ); ?>
								...
								<li>
									<a href="<?php echo apply_filters( 'widget_title', $instance['tw_link'] ); ?>"
									   target="<?php echo $onepage_team_target; ?>">
										...
										if ( ! empty( $instance['name'] ) ) { ?>
											<span class="sr-only">
				                            <?php _e( 'Twitter account of', 'mythemeshop-theme-customizer' ); ?>
				                            <?php echo apply_filters( 'widget_title', $instance['name'] ); ?>
			                            </span>
											<?php
										} ?>
								...
								<li>
									<a href="<?php echo apply_filters( 'widget_title', $instance['bh_link'] ); ?>"
									   target="<?php echo $onepage_team_target; ?>">
										...
										if ( ! empty( $instance['name'] ) ) { ?>
											<span class="sr-only">
				                            <?php _e( 'Behance account of', 'mythemeshop-theme-customizer' ); ?>
				                            <?php echo apply_filters( 'widget_title', $instance['name'] ); ?>
			                            </span>
											<?php
										} ?>
								...
								<li>
									<a href="<?php echo apply_filters( 'widget_title', $instance['db_link'] ); ?>"
									   target="<?php echo $onepage_team_target; ?>">
										...
										if ( ! empty( $instance['name'] ) ) { ?>
											<span class="sr-only">
				                            <?php _e( 'Dribble account of', 'mythemeshop-theme-customizer' ); ?>
				                            <?php echo apply_filters( 'widget_title', $instance['name'] ); ?>
			                            </span>
											<?php
										} ?>
								...
								<li>
									<a href="<?php echo apply_filters( 'widget_title', $instance['ln_link'] ); ?>"
									   target="<?php echo $onepage_team_target; ?>">
										...
										if ( ! empty( $instance['name'] ) ) { ?>
											<span class="sr-only">
				                            <?php _e( 'Linkedin account of', 'mythemeshop-theme-customizer' ); ?>
				                            <?php echo apply_filters( 'widget_title', $instance['name'] ); ?>
			                            </span>
											<?php
										} ?>
		...
		function form( $instance ) {
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'name' ); ?>"><?php _e( 'Name', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'position' ); ?>"><?php _e( 'Position', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'description' ); ?>"><?php _e( 'Description', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'fb_link' ); ?>"><?php _e( 'Facebook link', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'tw_link' ); ?>"><?php _e( 'Twitter link', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'bh_link' ); ?>"><?php _e( 'Behance link', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'db_link' ); ?>"><?php _e( 'Dribble link', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'ln_link' ); ?>"><?php _e( 'Linkedin link', 'mythemeshop-theme-customizer' ); ?></label><br/>
			...
			<p>
				<input type="checkbox" name="<?php echo $this->get_field_name( 'open_new_window' ); ?>"
				       id="<?php echo $this->get_field_id( 'open_new_window' ); ?>" <?php if ( ! empty( $instance['open_new_window'] ) ): checked( (bool) $instance['open_new_window'], true ); endif; ?> ><?php _e( 'Open links in new window?', 'mythemeshop-theme-customizer' ); ?>
				<br>
			</p>
			<p>
				<label
					for="<?php echo $this->get_field_id( 'image_uri' ); ?>"><?php _e( 'Image', 'mythemeshop-theme-customizer' ); ?></label><br/>
				...
				$onepage_image_in_customizer = $this->get_field_name( 'image_in_customizer' );
				?>
				<input type="hidden" class="custom_media_display_in_customizer"
				       name="<?php if ( ! empty( $onepage_image_in_customizer ) ) {
					       echo $onepage_image_in_customizer;
				       } ?>"
				...
				<img class="custom_media_image" src="<?php echo $image_in_customizer; ?>"
				     ...
				     alt="<?php echo __( 'Uploaded image', 'mythemeshop-theme-customizer' ); ?>"/><br/>
				...
				<input type="button" class="button button-primary custom_media_button" id="custom_media_button"
				       name="<?php echo $this->get_field_name( 'image_uri' ); ?>"
				       value="<?php _e( 'Upload Image', 'mythemeshop-theme-customizer' ); ?>" style="margin-top:5px;">
			</p>
